...
public class maze_2 {
	...
	ArrayList<Integer[]> secpath = new ArrayList<Integer[]>();
	ArrayList<Integer[]> pod = new ArrayList<Integer[]>();
	boolean shtrace=false;
	...
	public boolean canDown1(int z, int y){
		boolean cand;
		try{
			cand = maze[z][y+1]==' '||maze[z][y+1]=='+';
		}
		catch(ArrayIndexOutOfBoundsException h){
			cand = false;
		}
	...
	public boolean canUp1(int z, int y){
		boolean cand;
		try{
			cand = maze[z][y-1]==' '||maze[z][y-1]=='+';
		}
	...
	public boolean canLeft1(int z, int y){
		boolean cand;
		try{
			cand = maze[z-1][y]==' '||maze[z-1][y]=='+';
		}
		catch(ArrayIndexOutOfBoundsException h){
			cand = false;
		}
	...
	public boolean canRight1(int z, int y){
		boolean cand;
		try{
			cand = maze[z+1][y]==' '||maze[z+1][y]=='+';
		}
	...
	public void filler3(int w, int mnum, int c, int y){//System.out.println("W        is            :       " + w)
		boolean d = canDown1(c,y),u=canUp1(c,y),l=canLeft1(c,y),r=canRight1(c,y);
		System.out.println(toString());
		if(maze[c][y]==' '){
			boolean d1;//=c+1==cats.get(0)[0]&&y==cat[1]||c-1==cats.get(0)[0]&&y==cats.get(0)[1]||y+1==cats.get(0)[1]&&c==cats.get(0)[0]||y-1==cats.get(0)[1]&&c==cats.get(0)[0];
			//for(int w = 1;w<cats.size();w++){
	...
	public void tracer(int z,int y, int mx, int my, int dssd, int mn) throws IOException{
		...
		System.out.println(toString());
		for(int u=0;u<pod.size();u++){
			System.out.print(pod.get(u)[0] + "-" + pod.get(u)[1]);
		}
		System.out.println("");
		for(int u=0;u<secpath.size();u++){
			System.out.print(secpath.get(u)[0] + "-" + secpath.get(u)[1]);
		}
		System.out.println("");
		read.readLine();
		...
		if(shtrace&&!path.isEmpty()){
			secpath.add(path.get(path.size()-1));
		}
		...
		if(z+1==mx&&y==my||z-1==mx&&y==my||y+1==my&&z==mx||y-1==my&&z==mx){
			...
			if(!secpath.isEmpty()){
				for(int tmp = 0;tmp<secpath.size();tmp++){
					maze[secpath.get(tmp)[0]][secpath.get(tmp)[1]]='f';
					path.remove(path.size()-1);
					dead.add(new Integer[]{secpath.get(tmp)[0],secpath.get(tmp)[1]});
				}
				ArrayList<Integer[]> nwempty = (ArrayList<Integer[]>) empty.clone();
				int count=0;
				for(Integer[] j:nwempty){
					if(maze[j[0]][j[1]]==' '){
						count++;
					}else{
						empty.remove(count);
				...
				//	maze[path.get(tmp)[0]][path.get(tmp)[1]]='O';
				//}
				//System.out.println(toString());
				//altclean();
	...
	public void method1(int l,int j){
		//try{
			ded.clear();
			Integer[] temp;
			Integer[] inty;
	...
	public void mazerize() throws IOException{
		...
		//for(int l=0;l<cats.size();l++){
			...
			//for(int j=0;j<mice.size();j++){
				...
				if(!empty.isEmpty())
					tracer(cats.get(0)[0],cats.get(0)[1],mice.get(0)[0],mice.get(0)[1],0,0);
				cleaner();
				path.clear();
				dead.clear();
			}
